...
/**
 * Favorite teams, global across every league (not a per-league list) - a
 * personal preference, on-device only, same as StarredGamesRepository. The
 * full Team snapshot (name + logo) is persisted, not just the name, so My
 * Teams can render immediately without a fresh network round-trip - the
 * logo shown there could go stale if ESPN ever changes a crest, an
 * acceptable tradeoff for a favorites list capped at 3.
 */
